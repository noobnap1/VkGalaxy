...
// All plain floats/uints — no vec3, no alignment surprises.
// Your GLSL push_constant block MUST match this exactly:
//
//   layout(push_constant) uniform Params {
//       float time;           // offset  0
//       uint  numStars;       // offset  4
//       float starSize;       // offset  8
//       float dustSize;       // offset 12
//       float h2Size;         // offset 16
//       float h2Dist;         // offset 20
//       float galaxyOffsetX;  // offset 24
//       float galaxyOffsetY;  // offset 28
//       float galaxyOffsetZ;  // offset 32
//   } params;
//
//   vec3 galaxyOffset = vec3(params.galaxyOffsetX, params.galaxyOffsetY, params.galaxyOffsetZ);
//   vec3 worldPos = vec3(particle.pos.x, particle.height, particle.pos.y) + galaxyOffset;
//   gl_Position = camera.viewProj * vec4(worldPos, 1.0);
struct ParticleParamsVertGPU
{
	f32    time;           // offset  0
	uint32 numStars;       // offset  4
	f32    starSize;       // offset  8
	f32    dustSize;       // offset 12
	f32    h2Size;         // offset 16
	f32    h2Dist;         // offset 20
	f32    galaxyOffsetX;  // offset 24
	f32    galaxyOffsetY;  // offset 28
	f32    galaxyOffsetZ;  // offset 32
};
...
static void _draw_record_particle_commands(DrawState* s, DrawParams* params, VkCommandBuffer commandBuffer, uint32 frameIndex, uint32 imageIdx, qm::vec3 offset);
...
static uint32 s_frameIdx = 0;

// Add this as a static or member somewhere accessible
static double s_lastTime = 0.0;
static int32  s_frameCount = 0;

static void _draw_update_perf_display(DrawState* s)
{
	double now = glfwGetTime();
	double elapsed = now - s_lastTime;
	s_frameCount++;

	if (elapsed >= 0.25) // update 4x per second
	{
		double fps = s_frameCount / elapsed;
		double frameMs = (elapsed / s_frameCount) * 1000.0;

		char title[128];
		snprintf(title, sizeof(title), "VkGalaxy  |  %.0f fps  |  %.2f ms", fps, frameMs);
		glfwSetWindowTitle(s->instance->window, title);
...
bool draw_begin_frame(DrawState* s, DrawParams* params, uint32* outFrameIdx, uint32* outImageIdx)
{
	uint32 frameIdx = s_frameIdx;
	...
	VkResult imageAcquireResult = vkAcquireNextImageKHR(
		s->instance->device, s->instance->swapchain, UINT64_MAX,
	...
	qm::mat4 view = qm::lookat(params->cam.pos, params->cam.target, params->cam.up);
	qm::mat4 projection = qm::perspective(params->cam.fov, (f32)windowW / (f32)windowH, 0.1f, INFINITY);
	...
	*outImageIdx = imageIdx;
	return true;
}

void draw_render_galaxy(DrawState* s, DrawParams* params, uint32 frameIdx, uint32 imageIdx, qm::vec3 offset)
{
	_draw_record_particle_commands(s, params, s->commandBuffers[frameIdx], frameIdx, imageIdx, offset);
}

void draw_end_frame(DrawState* s, uint32 frameIdx, uint32 imageIdx)
{
	...
	s_frameIdx = (frameIdx + 1) % FRAMES_IN_FLIGHT;
	_draw_update_perf_display(s);
}

void draw_render(DrawState* s, DrawParams* params, f32 dt)
{
	uint32 frameIdx, imageIdx;
	if (!draw_begin_frame(s, params, &frameIdx, &imageIdx))
		return;

	draw_render_galaxy(s, params, frameIdx, imageIdx, { 0.0f, 0.0f, 0.0f });

	draw_end_frame(s, frameIdx, imageIdx);
}
...
static bool _draw_create_depth_buffer(DrawState* s)
{
	...
	s->finalDepthImage = vkh_create_image(s->instance, s->instance->swapchainExtent.width, s->instance->swapchainExtent.height, 1,
		VK_SAMPLE_COUNT_1_BIT, s->depthFormat, VK_IMAGE_TILING_OPTIMAL,
...
static bool _draw_create_grid_pipeline(DrawState* s)
{
	...
	vkh_pipeline_set_raster_state(s->gridPipeline, VK_FALSE, VK_FALSE, VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE, VK_FALSE, 0.0f, 0.0f, 0.0f);
...
static bool _draw_create_particle_pipeline(DrawState* s)
{
	...
	// Push constant size must match sizeof(ParticleParamsVertGPU) = 36 bytes
	VkPushConstantRange vertPushConstant = {};
	vertPushConstant.offset = 0;
	...
	vkh_pipeline_set_raster_state(s->particlePipeline, VK_FALSE, VK_FALSE, VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE, VK_FALSE, 0.0f, 0.0f, 0.0f);
...
static void _draw_record_particle_commands(DrawState* s, DrawParams* params, VkCommandBuffer commandBuffer, uint32 frameIndex, uint32 imageIdx, qm::vec3 offset)
{
	...
	// Fill push constants — plain floats, no vec3 alignment issues.
	// GLSL layout must match exactly (see ParticleParamsVertGPU comment above).
	ParticleParamsVertGPU vertParams;
	vertParams.time = (f32)glfwGetTime();
	...
	vertParams.galaxyOffsetX = offset.x;
	vertParams.galaxyOffsetY = offset.y;
	vertParams.galaxyOffsetZ = offset.z;
